    ...
    with get_db() as conn:
        ...
        today_picks = conn.execute("""
            SELECT p.*, g.home_team, g.away_team, g.home_starter_name, g.away_starter_name,
                   ...
                   g.status,
                   (SELECT fip FROM pitcher_stats WHERE player_id = g.home_starter_id AND fip IS NOT NULL ORDER BY season DESC LIMIT 1) as home_fip,
                   (SELECT era FROM pitcher_stats WHERE player_id = g.home_starter_id AND era IS NOT NULL ORDER BY season DESC LIMIT 1) as home_era,
                   (SELECT MAX(innings_pitched) FROM pitcher_stats WHERE player_id = g.home_starter_id) as home_ip,
                   (SELECT k_per_9 FROM pitcher_stats WHERE player_id = g.home_starter_id AND k_per_9 IS NOT NULL ORDER BY season DESC LIMIT 1) as home_k9,
                   ...
                   (SELECT fip FROM pitcher_stats WHERE player_id = g.away_starter_id AND fip IS NOT NULL ORDER BY season DESC LIMIT 1) as away_fip,
                   (SELECT era FROM pitcher_stats WHERE player_id = g.away_starter_id AND era IS NOT NULL ORDER BY season DESC LIMIT 1) as away_era,
